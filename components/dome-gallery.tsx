...
export default function DomeGallery({
  ...
  useEffect(() => {
    ...
    const step = () => {
      if (!draggingRef.current && !focusedElRef.current) {
        const nextY = wrapAngleSigned(rotationRef.current.y + autoRotateSpeed * 0.016)
  ...
  useGesture(
    {
      ...
      onDrag: ({ event, last, velocity: velArr = [0, 0], direction: dirArr = [0, 0], movement }) => {
        ...
        if (cur.x !== nextX || cur.y !== nextY) { rotationRef.current = { x: nextX, y: nextY }; applyTransform(nextX, nextY) }
        if (last) {
  ...
  useEffect(() => {
    ...
    const close = () => {
      ...
      if (!originalPos) {
        ...
        el.style.visibility = '';(el.style as CSSStyleDeclaration & { zIndex: string }).zIndex = '0'
        focusedElRef.current = null; rootRef.current?.removeAttribute('data-enlarging'); openingRef.current = false; return
      }
      ...
      requestAnimationFrame(() => {
        animatingOverlay.style.left = origRel.left + 'px'; animatingOverlay.style.top = origRel.top + 'px'
        animatingOverlay.style.width = origRel.width + 'px'; animatingOverlay.style.height = origRel.height + 'px'
      ...
      const cleanup = () => {
        ...
        parent.style.setProperty('--rot-y-delta', '0deg'); parent.style.setProperty('--rot-x-delta', '0deg')
        requestAnimationFrame(() => {
          el.style.visibility = ''; el.style.opacity = '0';(el.style as CSSStyleDeclaration & { zIndex: string }).zIndex = '0'
  ...
  const openItemFromElement = (el: HTMLElement) => {
    ...
    el.style.visibility = 'hidden';(el.style as CSSStyleDeclaration & { zIndex: string }).zIndex = '0'
    ...
    const img = document.createElement('img'); img.src = rawSrc; img.alt = rawAlt
    ...
    if (openedImageWidth || openedImageHeight) {
      const onFirstEnd = (ev: TransitionEvent) => {
        ...
        overlay.addEventListener('transitionend', () => { overlay.style.transition = prevTransition }, { once: true })
      }
            ...
            <div ref={sphereRef} className="sphere">
              {items.map((it, i) => (
                <div
                  ...
                  style={{
                    ['--offset-x' as string]: it.x, ['--offset-y' as string]: it.y,
                    ['--item-size-x' as string]: it.sizeX, ['--item-size-y' as string]: it.sizeY,
